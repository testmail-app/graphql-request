...
//     const err: ClientError = t.throws(await request('https://mock-api.com/graphql', `x`))
...
//   }
//   await mock({body: {errors}}, async () => {
//     const err: ClientError = t.throws(await rawRequest('https://mock-api.com/graphql', `x`))
//     t.deepEqual<any>(err.response.errors, errors)
//   })
// })
...
test('extra fetch options', async (t) => {
  ...
  await mock({
    ...
    if (!actualOptions) {
      throw new Error('actualOptions is undefined');
    }
